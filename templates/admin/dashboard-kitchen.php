                ...
                if (in_array($current_status, array('pending', 'processing'))) {
                    ...
                    try {
                        ...
                        if ($new_status === 'pending') {
                            error_log('Orders Jet Kitchen: SUCCESS - Order #' . $order_id . ' status changed to pending by user #' . get_current_user_id());
                            ...
                            error_log('Orders Jet Kitchen: ERROR - Status did not change as expected. Expected: pending, Actual: ' . $new_status);
                            $error_message = sprintf(__('Status change failed. Expected: pending, Got: %s', 'orders-jet'), $new_status);
                        }
...
if (function_exists('wc_get_orders')) {
    ...
    foreach ($wc_orders as $wc_order) {
        ...
        error_log('Orders Jet Kitchen DEBUG: Order #' . $wc_order->get_id() . ' - Delivery info: ' . ($has_delivery_time ? json_encode($delivery_info) : 'NONE'));
        ...
        if (!empty($table_number)) {
            ...
            error_log('Orders Jet Kitchen: TIMED pickup order #' . $wc_order->get_id() . ' - ' . $order_type_label);
        } else {
            ...
            error_log('Orders Jet Kitchen: REGULAR pickup order #' . $wc_order->get_id());
        }
...
// Get completed orders (pending) - ultra simple
if (function_exists('wc_get_orders')) {
    $completed_wc_orders = wc_get_orders(array(
        'status' => array('pending'), // Ready orders
        'limit' => -1
    ));
...
if (function_exists('wc_get_orders')) {
    ...
    if (isset($completed_wc_orders)) {
        error_log('Orders Jet Kitchen Stats: Found ' . count($completed_wc_orders) . ' pending orders');
                ...
                <?php elseif ($order['post_status'] === 'wc-processing') : ?>
                    ...
                    if ($delivery_info) {
                        ...
                        if ($countdown_info) {
                            ...
                            error_log('Orders Jet Kitchen: Order #' . $order['ID'] . ' Countdown Debug:');
                    ...
                    ?>
                                ...
                                <?php elseif ($order['post_status'] === 'wc-pending') : ?>
                                    <?php if (!empty($order['table_number'])) : ?>
                                        <span class="oj-status-badge pending-table">
                                            <span class="dashicons dashicons-money-alt"></span>
                                            <?php _e('Ready - Awaiting Payment', 'orders-jet'); ?>
                                        </span>
                                    <?php else : ?>
                                        <span class="oj-status-badge pending-pickup">
                                            <span class="dashicons dashicons-money-alt"></span>
...
.oj-status-badge.pending-table {
    background: #fff3cd;
    color: #856404;
    border: 1px solid #ffeaa7;
}

.oj-status-badge.pending-pickup {
    background: #d4edda;
    color: #155724;
